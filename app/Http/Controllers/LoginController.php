...
class LoginController extends Controller
{
    public function HandleLoginContoller(Request $request){
        ...
        $user = DB::select( DB::raw("SELECT staff_id,username, password,account_type FROM user WHERE username ='$user_entered_username' AND account_type='$user_entered_login_type'"));

        foreach($user as $i){

           $real_staff_id     =     $i->staff_id;
           $real_username     =     $i->username;
           $real_password     =     $i->password;
           $real_account_type =     $i->account_type;
       ...
       if($real_username != "" && $real_password != "" && $real_account_type != ""){
         ...
         return Redirect::to("/")->withErrors(['no account found for this credentials']);
       }
